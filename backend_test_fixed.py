...
class BackendTester:
    ...
    def run_all_tests(self):
        ...
        if response and response.status_code == 200:
            ...
            self.log_test("Root Endpoint", False, f"Failed: {response.status_code if response else 'No response'}")
        ...
        # Test 2: Super Admin Creation (should fail)
        ...
        # Invalid login
        invalid_login = {"username": "superadmin", "password": "wrongpassword"}
        ...
        # Non-existent user
        nonexistent_login = {"username": "nonexistentuser", "password": "anypassword"}
        ...
        if response is None:
            self.log_test("Get Current User (No Token)", False, "No response from server")
        elif response.status_code == 403:
            self.log_test("Get Current User (No Token)", True, "Correctly rejected request without token")
        else:
            self.log_test("Get Current User (No Token)", False, f"Unexpected status code: {response.status_code}")
        
        # With invalid token
        response = self.make_request("GET", "/auth/me", token="invalid_token_12345")
        if response is None:
            self.log_test("Get Current User (Invalid Token)", False, "No response from server")
        elif response.status_code == 401:
            self.log_test("Get Current User (Invalid Token)", True, "Correctly rejected invalid token")
        else:
            self.log_test("Get Current User (Invalid Token)", False, f"Unexpected status code: {response.status_code}")
        ...
        if response is None:
            self.log_test("Super Admin Creates Admin", False, "No response from server")
            ...
            if data.get("username") == "rajesh_admin" and data.get("role") == "admin":
                ...
                admin_login = {"username": "rajesh_admin", "password": "SecurePass123!"}
            ...
            if "already registered" in data.get("detail", ""):
                self.log_test("Super Admin Creates Admin", True, "Admin user already exists (from previous test)")
                ...
                if login_response and login_response.status_code == 200:
                    ...
                    self.tokens["rajesh_admin"] = login_data["access_token"]
                    self.users["rajesh_admin"] = login_data["user"]
                    self.log_test("Existing Admin Login", True, "Existing admin can login successfully")
            else:
                self.log_test("Super Admin Creates Admin", False, "Registration failed", data)
        else:
            self.log_test("Super Admin Creates Admin", False, f"Registration failed: {response.status_code}")
        
        # Super Admin tries to create Karyakarta (should fail)
        karyakarta_data = {
        ...
        if response is None:
            self.log_test("Super Admin Creates Karyakarta (Should Fail)", False, "No response from server")
        elif response.status_code == 403:
            data = response.json()
            ...
            self.log_test("Super Admin Creates Karyakarta (Should Fail)", False, f"Unexpected status code: {response.status_code}")
        
        # Admin creates Karyakarta
        if "rajesh_admin" in self.tokens:
            ...
            if response is None:
                self.log_test("Admin Creates Karyakarta", False, "No response from server")
            elif response.status_code == 201:
                data = response.json()
                if data.get("username") == "amit_karyakarta" and data.get("role") == "karyakarta":
                    ...
                    karyakarta_login = {"username": "amit_karyakarta", "password": "SecurePass123!"}
                ...
                data = response.json()
                if "already registered" in data.get("detail", ""):
                    self.log_test("Admin Creates Karyakarta", True, "Karyakarta user already exists (from previous test)")
                    # Try to login with existing karyakarta
                    karyakarta_login = {"username": "amit_karyakarta", "password": "SecurePass123!"}
                    login_response = self.make_request("POST", "/auth/login", karyakarta_login)
                    if login_response and login_response.status_code == 200:
                        login_data = login_response.json()
                        self.tokens["amit_karyakarta"] = login_data["access_token"]
                        self.users["amit_karyakarta"] = login_data["user"]
                        self.log_test("Existing Karyakarta Login", True, "Existing karyakarta can login successfully")
                else:
                    self.log_test("Admin Creates Karyakarta", False, "Registration failed", data)
            else:
                self.log_test("Admin Creates Karyakarta", False, f"Registration failed: {response.status_code}")
            ...
            if response is None:
                self.log_test("Admin Creates Admin (Should Fail)", False, "No response from server")
            elif response.status_code == 403:
                data = response.json()
                ...
                self.log_test("Admin Creates Admin (Should Fail)", False, f"Unexpected status code: {response.status_code}")
        ...
        # Admin sees only their Karyakartas
        if "rajesh_admin" in self.tokens:
        ...
        if "rajesh_admin" not in self.tokens or "amit_karyakarta" not in self.users:
            ...
            if not karyakarta_id:
                self.log_test("User Deactivation Setup", False, "Could not find karyakarta ID")
            else:
                # Admin deactivates their Karyakarta
                response = self.make_request("PUT", f"/auth/users/{karyakarta_id}/deactivate", token=self.tokens["rajesh_admin"])
                if response is None:
                    self.log_test("Admin Deactivates Karyakarta", False, "No response from server")
                    ...
                    if "deactivated successfully" in data.get("message", ""):
                        ...
                        deactivated_login = {"username": "amit_karyakarta", "password": "SecurePass123!"}
                        login_response = self.make_request("POST", "/auth/login", deactivated_login)
                        if login_response is None:
                            self.log_test("Deactivated User Login", False, "No response from server")
                            ...
                            self.log_test("Deactivated User Login", False, f"Unexpected status code: {login_response.status_code}")
                    else:
                        self.log_test("Admin Deactivates Karyakarta", False, "Unexpected response message", data)
                else:
                    self.log_test("Admin Deactivates Karyakarta", False, f"Deactivation failed: {response.status_code}")
